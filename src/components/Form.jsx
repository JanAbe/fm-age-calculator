  ...
  return (
    <>
      <form className="age-form" onSubmit={handleSubmit}>
        <Input showError={!dayIsValid} errorMessage="Day must be between 1-31" placeholder="DD" label="day" onChange={(e) => setDay(e.target.value)}/>
        <Input showError={!monthIsValid} errorMessage="Month must be between 1-12" placeholder="MM" label="month" onChange={(e) => setMonth(e.target.value)}/>
        <Input showError={!yearIsValid} errorMessage="Year can't be in the future" placeholder="YYYY" label="year" onChange={(e) => setYear(e.target.value)}/>
